...
# --- BLINDAGEM CONTRA PERDA DE DADOS NO F5 (RACE CONDITION) ---
if not st.session_state.get('memoria_carregada'):
    memoria_bruta = local_storage.getItem("pap_memoria_v3")
    
    if memoria_bruta is None:
        # Trava absoluta: O navegador ainda não devolveu os dados. 
        # O script PARA AQUI para não renderizar as listas vazias e sobrescrever o cache.
        st.caption("⏳ Sincronizando dados locais do aparelho...")
        st.stop()
    else:
        # O navegador devolveu a resposta (pode ser os dados ou string vazia)
        if memoria_bruta:
            try:
...
# --- API GOOGLE CRUD COM PROTEÇÃO CONTRA CRASH DE JSON ---
def api_google(payload):
    try:
        ...
        return {"status": "erro", "msg": "Tempo esgotado. A internet pode estar fraca."}
    except Exception as e: 
        return {"status": "erro", "msg": f"Falha na comunicação com o servidor Google: {str(e)}"}
...
# Filtra dinamicamente as linhas reais da planilha (Ignora cabeçalhos baseando-se no ID Protocolo)
linhas_reais = []
if st.session_state['crm_dados']:
    for linha in st.session_state['crm_dados']:
        # Se a linha tiver pelo menos 20 colunas e o índice 1 (Protocolo) começar com 'PAP', é um dado real
        if len(linha) > 19 and str(linha[1]).startswith("PAP"):
            linhas_reais.append(linha)

for l in linhas_reais:
    # Índice 21 é o Vendedor e 19 é o Status, baseado exatamente no seu script doPost
    if len(l) > 21 and str(l[21]) == st.session_state['vendedor_atual']:
...
# ================= MÓDULO 3: GESTÃO CRM =================
    ...
    if not linhas_reais:
        ...
        # Filtra as linhas pelo vendedor atual (Índice 21 do seu doPost)
        linhas_vendedor = [l for l in linhas_reais if len(l) > 21 and str(l[21]) == st.session_state['vendedor_atual']]
        ...
        for linha in linhas_vendedor:
            # Garante que a linha tenha todos os slots preenchidos até o final (Evita IndexError no valor recebido)
            while len(linha) < 23: linha.append("")

            prot = linha[1]  # Protocolo fixo
            nome_c = linha[2]
            whats_c = linha[6]
            op_c = linha[13]
            plano_c = linha[14]
            status_raw = str(linha[19]).strip() # Status fixo
            status_clean = status_raw.lower()
            data_c = str(linha[0])[:10]
            val_recebido = linha[22]
            ...
            if mostrar:
                ...
                with c_act:
                    ...
                    if col_b1.button("Salvar", key=f"sv_{prot}"):
                        linha[19] = novo_st
                        if novo_st == "Instalada": linha[22] = blindar_texto(novo_val)

                        payload = {"acao": "editar", "senha_api": SENHA_MESTRE_GESTAO, "id_busca": prot,
                                   "coluna_busca": 1, "novos_dados": linha}
                        ...
                        with st.spinner("Processando..."):
                            ...
                            if resposta and resposta.get('status') == 'sucesso':
                                ...
                                erro_msg = resposta.get('msg', 'Desconhecido') if resposta else "Falha de rede"
                                m_erro(f"Falha na gravação na planilha: {erro_msg}")
    ...
    if not st.session_state['modo_gestao_liberado']:
        ...
        with tb2:
            ...
            if not linhas_reais:
                ...
                linhas_admin = [l for l in linhas_reais if len(l) > 21 and str(l[21]) == st.session_state['vendedor_atual']]
                lucro_total = 0.0
